...
const mapStateToProps = (state, ownProps) => {
  const categories = (Boolean(state.wp.courseCategories.ids) &&
    state.wp.courseCategories.ids.map(id => state.entities.course_categories[id])) || [];
  const courses = (Boolean(state.wp.courses.ids) &&
    state.wp.courses.ids.map(id => state.entities.courses[id])) || [];
